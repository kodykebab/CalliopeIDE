...
def get_project_files(current_user, project_id):
    ...
    try:
        ...
        def build_tree(path, base_path):
            ...
            """Recursively build file tree structure with security validation"""
            ...
            try:
                ...
                abs_path = os.path.realpath(str(path_obj))

                abs_base = os.path.realpath(base_path)

                if not abs_path.startswith(abs_base):

                    logger.warning(f"Path traversal attempt: {abs_path} outside {abs_base}")
